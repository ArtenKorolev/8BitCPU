...
void cpu_move_to_register_immediate(cpu_t *self, byte_t *register_ptr, char register_name, memory_t *memory);
void cpu_add_immediate_to_register_A(cpu_t *self, memory_t *memory);
...
void cpu_exec(cpu_t *self, memory_t *memory, byte_t fetched_byte) {
  ...
  switch (fetched_byte) {
    case NOOP_OPCOD:
      ...
      break;
    case MOVAI_OPCOD:
      cpu_move_to_register_immediate(self, &self->regA, 'A', memory);
      break;
    case MOVXI_OPCOD:
      cpu_move_to_register_immediate(self, &self->regX, 'X', memory);
      break;
    case MOVYI_OPCOD:
      cpu_move_to_register_immediate(self, &self->regY, 'Y', memory);
      break;
    case MOVZI_OPCOD:
      cpu_move_to_register_immediate(self, &self->regZ, 'Z', memory);
      break;
    case ADDI_OPCOD:
      cpu_add_immediate_to_register_A(self, memory);
      break;
    case JMP_OPCOD:
...
void cpu_move_to_register_immediate(cpu_t *self, byte_t *register_ptr, const char register_name, memory_t *memory) {
  ...
  const byte_t immediate = cpu_fetch(self, memory, &success);
...
void cpu_add_immediate_to_register_A(cpu_t *self, memory_t *memory) {
  ...
  if (success) {
    if (check_8bit_overflow(self->regA, immediate)) {
